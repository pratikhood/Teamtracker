...
public class TC_Modify_Manager_011 extends BaseClass
{

	@Test(dataProvider="Data")
	public void Modify_Manager(String name,String num, String email,String empid,String username, String modified_name) throws InterruptedException
	{
		...
		for(int i=1;i<=count_of_managers;i++)
		{
			...
			if (manager_names.contains("Ramesh Chaudhary")) 
			{ 
				...
				manager_page.click_on_modify_button();
				
				manager_page.clear_manager_name();
				
				manager_page.enter_modified_manager_name(modified_name);
				
				manager_page.click_on_save_button_while_modify ();
...

	{
		...
		int rownum=XLUtils.getRowCount(path, "Modify Manager");
		int colcount=XLUtils.getCellCount(path,"Modify Manager",1);
		...
		for(int i=1;i<=rownum;i++)
		{
			...
			{
				logindata[i-1][j]=XLUtils.getCellData(path,"Modify Manager", i,j);//1 0
			}
