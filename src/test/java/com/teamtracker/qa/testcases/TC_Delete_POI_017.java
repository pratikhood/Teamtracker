...
public class TC_Delete_POI_017 extends BaseClass
{



	@Test(dataProvider="Data")
	public void Delete_POI(String poi_name, String poi_addr) throws InterruptedException
	{
		...
		String y = "]/td[5]//span[@class='fa fa-trash']";
		...
		for(int i=1;i<=count_of_POIs;i++)
		{
			...
			if (POIs_names.contains("Testing POI")) 
			{ 
...

	{
		...
		int rownum=XLUtils.getRowCount(path, "Delete POI");
		int colcount=XLUtils.getCellCount(path,"Delete POI",1);
		...
		for(int i=1;i<=rownum;i++)
		{
			...
			{
				logindata[i-1][j]=XLUtils.getCellData(path,"Delete POI", i,j);//1 0
			}
